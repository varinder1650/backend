...
# ✅ Improved geocoding with better validation
async def get_coordinates_from_address(
    ...
    """
    Geocode an address to get latitude and longitude with improved accuracy
    Returns: (latitude, longitude) or (None, None) if failed
    """
    try:
        # ✅ Build address with pincode priority (more accurate in India)
        # Format 1: Try with pincode first (most accurate)
        address_formats = [
            f"{street}, {city}, {pincode}, India",  # Most accurate
            f"{street}, {city}, {state}, {pincode}, India",  # Full address
            f"{pincode}, {city}, {state}, India",  # Pincode-based fallback
        ]
        
        for full_address in address_formats:
            logger.info(f"🗺️ Attempting geocoding: {full_address}")
            
            url = f"{OLA_BASE_URL}/geocode"
            params = {
                'address': full_address,
                'api_key': OLA_API_KEY,
                'bounds': '6.4,68.7|35.6,97.25',  # India bounds for better accuracy
                'language': 'en'
            }
            ...
            async with httpx.AsyncClient(timeout=15.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    if data.get('geocodingResults') and len(data['geocodingResults']) > 0:
                        ...
                        if lat and lng:
                            if 6.4 <= lat <= 35.6 and 68.7 <= lng <= 97.25:
                                logger.info(f"✅ Valid coordinates found: {lat}, {lng}")
                                logger.info(f"📍 Formatted address: {result.get('formatted_address')}")
                                return (lat, lng)
                            else:
                                logger.warning(f"⚠️ Coordinates outside India: {lat}, {lng}")
                                continue  # Try next format
                    
                await asyncio.sleep(0.5)  # Small delay between attempts
        
        logger.warning(f"⚠️ Could not geocode address after all attempts")
        return (None, None)
...
def validate_coordinates(lat: Optional[float], lng: Optional[float]) -> bool:
    """Check if coordinates are valid and within India"""
    if lat is None or lng is None:
        return False
    if lat == 0 and lng == 0:
        return False
    # Check if within India bounds
    if not (6.4 <= lat <= 35.6 and 68.7 <= lng <= 97.25):
        return False
    return True
...
async def create_address(
    ...
    try:
        ...
        provided_lat = address_data.latitude
        provided_lng = address_data.longitude
        
        # Check if provided coordinates are valid
        if validate_coordinates(provided_lat, provided_lng):
            logger.info(f"📍 Using provided valid coordinates: {provided_lat}, {provided_lng}")
            address_doc["latitude"] = provided_lat
            address_doc["longitude"] = provided_lng
        else:
            # Coordinates invalid or missing - geocode the address
            logger.info("🔍 Invalid or missing coordinates, attempting to geocode address...")
            lat, lng = await get_coordinates_from_address(
                address_data.street,
                address_data.city,
                address_data.state or "",
...
async def set_default_address(
    ...
    try:
        ...
        await db.update_one(
            "user_addresses",
            {"_id": ObjectId(address_id)},
            {"$set": {"is_default": True, "updated_at": now_utc()}}
        )
