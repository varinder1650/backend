...
#         access = create_access_token(
#             data={'sub': str(custom_id), 'role': user.get('role', 'customer')},
#             exp_time=timedelta(minutes=int(os.getenv('ACCESS_TOKEN_EXPIRE_MINUTES', 30)))
#         )
#         refresh = await create_refresh_token(str(custom_id), db)
...
#         # ✅ ADD DEBUG - Check what field exists in users collection
#         logger.info(f"🔍 Looking for user with id: {user_id}")
        
#         # Try both fields to debug
#         user_by_id = await db.find_one("users", {"id": user_id})
#         user_by_object_id = None
        
#         if ObjectId.is_valid(user_id):
#             user_by_object_id = await db.find_one("users", {"_id": ObjectId(user_id)})
        
#         logger.info(f"🔍 User found by 'id': {user_by_id is not None}")
#         logger.info(f"🔍 User found by '_id': {user_by_object_id is not None}")
        
#         user = user_by_id or user_by_object_id
...
#         # Create new access token - use the custom id from the user document
#         access_token = create_access_token(
