...
async def add_to_cart(
    ...
    try:
        ...
        # Handle PRODUCTS (existing logic)
            ...
            if not req.id or not InputValidator.validate_custom_id(req.id):
            ...
            product = await db.find_one("products", {"id": req.id, "is_active": True})
            ...
            existing_item = next(
                (i for i in cart["items"] 
                 if i.get("id") == req.id and i.get("serviceType") != "porter" and i.get("serviceType") != "printout"), 
                None
            )
...
async def batch_add_to_cart(
    ...
    product_items = [i for i in items if i.serviceType == 'product' and i.id]
    ...
    try:
        ...
        for i in product_items:
            product = product_map.get(i.id)
            if not product: 
                stock_errors.append(f"Product {i.id} not found")
        ...
        for i in product_items:
            existing_item = next(
                (it for it in cart["items"] 
                 if it.get("product") == i.id and it.get("serviceType") == "product"), 
                None
            )
            if existing_item:
...
async def get_cart(
    ...
    try:
        ...
        product_ids = [item["id"] for item in cart["items"] if item.get("id")]
        products = await db.find_many("products", {"id": {"$in": product_ids}, "is_active": True})
        product_map = {p["id"]: p for p in products}
...
async def update_cart_item(
    ...
    try:
        ...
        cart = await get_cart_or_create(db, current_user.id)
        item = next((i for i in cart["items"] if i.get("id") == req.itemId), None)
        ...
        # Check product stock
        product = await db.find_one("products", {"id": item["id"], "is_active": True})
        if not product: 
            raise HTTPException(status_code=404, detail="Product not found or inactive")
        if product.get("stock", 0) < req.quantity: 
            raise HTTPException(
