...
class OrderService:
    ...
    #                 "service_data": {
    #                     **item.service_data,
    #                     "file_urls": item.service_data.file_urls
    #                 }
    #             })
    #         else:
    ...
    async def create_order(self, order_data: OrderCreate, current_user, id: str):
        print("=============")
        print("order_data: ", order_data)
        order_dict = order_data.dict()  # or model_dump() for Pydantic v2
        order_dict['user'] = current_user.id
        order_dict['id'] = id
        order_dict['created_at'] = datetime.utcnow()
        order_dict['status_change_history'] = [{
        ...
        for item in order_data.items:
            if item.type == "product":
                ...
                processed_items.append({
                    "type": "printout",
                    "service_data": service_data_dict  # ✅ Already a dict, no need for **
                })
            else:
                raise ValueError(f"Unknown item type: {item.type}")
